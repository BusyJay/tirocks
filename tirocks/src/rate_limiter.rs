...
impl RateLimiter {
    ...
    /// Requests token to read or write bytes.
    /// 
    /// Takes into account SingleBurstBytes() and alignment (e.g., in case of
    /// direct I/O) to allocate an appropriate number of bytes, which may be less
    /// than the number of bytes requested.
    #[inline]
    pub fn request_token(&self, bytes: usize, align: usize, pri: IoPriority, op_ty: OpType) -> usize {
        unsafe {
            tirocks_sys::crocksdb_ratelimiter_request_token(self.ptr, bytes, align, pri, op_ty)
        }
    }
...
mod tests {
    ...
    fn test_rate_limiter() {
        ...
        assert!(actual_speed < speed * 1.5 && actual_speed > speed * 0.5, "{}", actual_speed);
    }
}
